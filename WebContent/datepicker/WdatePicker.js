...
if(IsPC()||bwdate){
	$(function(){
		for (var i = 0; i < $(".Wdate").length; i++) {
			$($(".Wdate")[i]).attr("type","text");
		}
	});
var $dp, WdatePicker;
(function() {
	...
	function I(G, _) {
		...
		function E(K, J) {
			var I = V[N].domain, F = false, G = "<iframe hideFocus=true width=300 height=300 frameborder=0 border=0 scrolling=no src=\"about:blank\"></iframe>";
			K.innerHTML = G;
					...
					if (D[C].name == J.skin)
						$
								.push("<link rel=\"stylesheet\" type=\"text/css\" href=\""
										...
										+ "skin/"
										+ D[C].name
										+ "/datepicker.css?ver=005\" charset=\""
										+ D[C].charset + "\"/>");
