...
//                    "<button class='btn_color' style='font-size: 16px;'>设置颜色</button>"+ 
			 ...
			 * @param url 文件地址
			 */
			function loadEditDig(id,url,fun){///wordedit/artical_dialog.jsp
			...
			function loadArtical(fun){
				$.get("artical_dialog.jsp?ver="+Math.random(),function(data){
			...
			var login_url="../pc/login-yuangong.jsp";
			$.ajaxSetup({
						...
						if (params) {
							...
							if (url&&url!=""&&url!="#") {
								$.get("article/"+type+"/"+url+"?math="+Math.random(),function(data){
									data=data.replace("/o2oindex",projectName);
									data=data.replace("o2oindex", projectName);
//									data=data.replace("/pc",projectName);
									data=data.replace("pc",projectName);
//									data=data.replace("/p1",projectName);
									data=data.replace("p1",projectName);
									text.html(data);
						...
						iframeContent.find("#save").click(function(){
							...
							saveArticle(text,url,type,article,articleaddress,function(data){
								...
								postArtical(type,typeName,title,wdate,publisher,htmlname,gjc.val(),path,poster,articleedit_content,zhiding,function(returl){
//									if(type!=4){
//										if(iframeContent.find("input[name='filetype']:checked").val()=="0"){
...
//									}
			...
			ahtml.find(".btn_del").bind("click",function(){
				if (confirm("是否要删除该文档内容!")) {
					var par=$(this).parents(".right-main-item");
