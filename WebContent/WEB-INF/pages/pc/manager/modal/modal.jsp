...
<div class="modal" style="display:block; top: 5%;">
			          ...
			          <div class="form-group">
			            <div class="input-group m-t-b">
							...
							<span class="input-group-btn">
						        <button class="btn btn-success btn-sm modalquery" type="button">搜索</button>
						    </span>
						</div>
						<button type="button" class="btn btn-primary btn-sm m-t-b">确定</button>
						<button type="button" class="btn btn-default btn-sm m-t-b">关闭</button>
	...
	function getMoalTr(data){
		return "<tr><td><div class='checkbox'></div></td><td>"+data.name+"</td><td>"+data.bm+"</td></tr>";
	}
	var tbody=$(".modal-dialog").find("tbody");
	$(".nav-tabs>li:eq(1)").click(function(){
		if($.trim(tbody.html())!=""){
			return;
		}
		pop_up_box.loadWait(); 
		$.get("getEmployeeRelevantSelect.do",{"type":${requestScope.type}},function(data){
			tbody.html("");
			$.each(data,function(i,n){
				tbody.append(getMoalTr(n));
			});
		});
	});
	$(".modalquery").click(function(){
		pop_up_box.loadWait(); 
		$.get("getEmployeeRelevantSelect.do",{
