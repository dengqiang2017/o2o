    	...
    	<div class="product-ctn">
          ...
          <div class="product-text">
           <div style="display: none;" id="com_id">${requestScope.product.com_id}</div>
                ...
                <div class="product-value" style="display:none;">对外标价:<span id="price_display">&yen;
                <fmt:formatNumber value="${requestScope.product.price_display}" pattern="0.00"/></span></div>
    			<div class="pstyle">
...
<%--                     <span>(如果库存数不足,供货周期为:<span>${requestScope.product.qz_days}</span>天)</span> --%>
    ...
    <span class="pro-num-U" style="margin-left: 4.3%;">(${requestScope.product.casing_unit})</span>
                ...
                <div class="ctn" style="margin-top:30px;line-height: 50px;">
                    <a class="btn btn-danger" id="orderpay">立即购买</a>
                    <a class="btn btn-danger" id="addShopping">加入购物车</a>
                    <a href="../pc/shopping.html" class="btn btn-danger">查看购物车</a>
                    <a class="btn btn-danger home_footer_left">咨询</a>
                </div>
                <div class="ctn" style="display: none;">
                    <a class="btn btn-danger" id="back" href="javascript:history.go(-1);">返回</a>
                </div>
    		</div>
    	</div>
    	<%--<div class="row">--%>
...
<!--                 <span class="pro-package-label">包装内容:</span> -->
                <span class="pro-package-content">${requestScope.product.item_detailspec_cn}</span>
            </div>
        </div>
        <div class="tabs-content" style="margin-bottom:20px">
            <div class="pro-package" id="pingjia">
                <c:forEach items="${requestScope.evals}" var="eval">
                <div style="height: 100px;border-bottom : 1px solid #ccc;margin-top: 10px;">
                	<div class="xingji">${eval.eval.spzl}</div><br>
                	<div>${eval.eval.yijian}</div>
                	<div>
                	 <c:forEach items="${eval.imgs}" var="img">
                	<img class="pingjia_img" src="${img}" onclick="showImg(this);" style="width: 50px;height: 50px;float: left;margin-left: 10px;">
                	 </c:forEach>
                	</div>
                </div>
                </c:forEach>
...
function showImg(t){
	$(".image-zhezhao").show();
	$("#imshow>img").attr("src",$(t).attr("src"));
}
...
$(".img-right").click(function(){
	var imgs=$("#pingjia").find(".pingjia_img");
	var len=imgs.length;
	if(index>=len){
		index=0;
	}else{
...
//
var xingji=$(".xingji");
for (var i = 0; i < xingji.length; i++) {
	var l=$(xingji[i]).html();
	$(xingji[i]).html("");
	for (var j = 0; j < l; j++) {
		$(xingji[i]).append("<img src='../pc/images/star.png' style='width: 15px;height: 15px;float: left;margin-left: 10px;'>");
	}
}
