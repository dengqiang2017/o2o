...
<!-- 					//////树形列表区 -->
					...
					<li class="parent_li"><span  class="reg_tree"><i class="glyphicon glyphicon-leaf"></i>${operate.com_sim_name}
...
operate={
		init:function(func){
			...
			operate.slectTreeVal(treeId,"行政区划",function(){
				if(func){
					func(treeId,treeName);
				}
				}); 
			operate.operate_init(); 
		},
		operate_init:function(){
			...
			$(".tabs-content").find(".reg_tree").click(function(){
				...
				operate.regioninit(this,function(n){
					return operate.treeli(n.com_sim_name_cn,n.com_id);
				});
			});
		},
		...
		regioninit:function(t,treeli){
			...
			if(t.next("ul").length>0){
				...
				pop_up_box.loadWait();
				$.get("../tree/getTree.do",{"treeId":treeId,"type":"operate","ver":Math.random()},function(data){
					var ulli="";
					if (data.length>1) {
						ulli="<ul>";
					}
