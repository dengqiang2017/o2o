var customer_id;
var imgPath;
$.get("../weixin/htmltourl.do",function(data){
	if(data.msg){
		if (data.msg.indexOf("redirect")>=0) {
			if(data.msg.indexOf("..")>=0){
				window.location.href=data.msg.split("redirect:")[1];
			}else{
				window.location.href=".."+data.msg.split("redirect:")[1];
			}
		}else{
...
$.get("../customer/getCustomerInfo.do",function(data){
    ...
    $("#user_logo").error(function(){
    	$("#user_logo").attr("src","touxiang.png");
    });
});
    ...
    function editfhdz(t){
    	...
    	var lxr=$.trim(li.find("#lxr").val());
		   ...
		   var fhdz=$.trim(li.find("#fhdz").val());
            ...
            if(lxr!=""&&lxPhone!=""&&fhdz!=""){
         	   savefhdz();
				li.find('input').addClass('input_style');
				li.find('textarea').addClass('input_style');
				...
				li.find('input').removeClass('input_style');
				li.find('textarea').removeClass('input_style');
    	...
    	if( $(this).text()=="确认"){
        	...
        	$.post("../customer/saveUserInfo.do",{
        		...
        		if (data.success) {
    				...
    				$('.mymaterial_list').find("input[type='tel'],input[type='text']").addClass('input_style');
					$('.mymaterial_list').find("select").addClass('input_style');
			...
			$('.mymaterial_list').find("input[type='tel'],input[type='text']").removeClass('input_style');
			$('.mymaterial_list').find("select").removeClass('input_style');
