...
var nicUploadButton = nicEditorAdvancedButton.extend({
	 nicURI: "../upload/uploadWordImg.do?templateName=pc&type="+window.parent.edithtml.type,
    errorText: "上传图片失败",
    addPane: function() {
       // if (typeof window.FormData === "undefined") {
      //      return this.onError("图片上传不支持使用这个浏览器，请使用谷歌，火狐，苹果,360等浏览器")
       // }
        this.im = this.ne.selectedInstance.selElm().parentTag("IMG");
        var A = new bkElement("div").setStyle({
            padding: "10px"
    ...
    uploadFile: function() {
        ...
        if (!B || (!B.type.match(/image.*/)&&B.type!="video/mp4")) {
            this.onError("只能上传图片或者mp4格式视频");
            return
        }
    ...
    onUploaded: function(B) {
        ...
        if (!this.im) {
        	...
        	$(this.im).css("width","400px");
        	$(this.im).css("height","400px");
            if(!browser.versions.gecko){
            	$(".nicEdit-main").find("img").bind("click",function(){
            		window.parent.edithtml.selectImg=$(this);
            		$("#imgwidth").val($(this).css("width").replace("px",""));
            		$("#imgheight").val($(this).css("height").replace("px",""));
            	});
            }
        } 
...
//            this.im.setAttributes({
//                src: D,
//                width: (A && B.image.width) ? Math.min(A, B.image.width) : "300"
//            })
//        }
    }
});
...
$(function(){
		$("#imgwidth").bind("input propertychange blur",function(){console.debug(window.parent.edithtml.selectImg);
			if(window.parent.edithtml.selectImg){
				window.parent.edithtml.selectImg.css("width",$(this).val()+"px");
			}else{
				pop_up_box.toast("请先在右边编辑器中点选图片!",1000);
				$(this).val("");
			}
		});
		$("#imgheight").bind("input propertychange blur",function(){
			if(window.parent.edithtml.selectImg){
				window.parent.edithtml.selectImg.css("height",$(this).val()+"px");
			}else{
				pop_up_box.toast("请先在右边编辑器中点选图片!",1000);
				$(this).val("");
			}
	...
	if(browser.versions.gecko){
		$("#imgwidth").parents(".form_group").hide();
		$("#imgheight").parents(".form_group").hide();
	}
});