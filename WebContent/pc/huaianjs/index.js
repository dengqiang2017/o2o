...
//			    });
			...
			function selectUnit(item){
				item.find('.xs_btn button').click(function(){
			        var c=$(this).hasClass('btn-danger');
			        $(this).parents(".pro-msg").find('.xs_btn button').removeClass("btn-danger");
			...
			$("#orderpay").click(function(){
				...
				if (chekcs&&chekcs.length>0) {
					...
					for (var i = 0; i < chekcs.length; i++) {
						...
						var index=item.find('.xs_btn button').index(item.find(".btn-danger"));
...
//							sd_unit_price=numformat2((1/pack_unit)*sd_unit_price);
			    ...
			    $('.home_footer_left').click(function(){
			        $('.kefu').slideToggle();
			    })