...
var clientAndEle={
		...
		init:function(){
			...
			$.get("../user/chatGet.do",{"chatid":chatid},function(data){
				if (data.name) {
					var info=data;
					$(".info").html(data.name+"("+data.userlist.length+")");
					avatar=data.avatar;
					username=data.username;
					clientAndEle.index=setInterval(function(){
...
//				$(".view-history").hide();
			...
			$("#clientLiaotian").find(".btn-primary").click(function(){
				pop_up_box.loadWait();
...
//					"anz_datetime":anz_datetime,
					...
					pop_up_box.loadWaitClose();
					$('a[data-title="title"]').html("电工安装费用支付");
