function getContainerHtml(url){
	...
	if(url.indexOf(".jsp")>0){
		$.get("../pc/saiyu/"+url,function(data){
...
$(function(){
	...
	if(url){
		...
		if(params[0]=="order"||url.indexOf("order")>=0){
			getContainerHtml('historicalPurchase');
		}else if(params[0]=="baoxiu"||url.indexOf("SPNR")>=0||url.indexOf("baoxiu")>=0){
			getContainerHtml('myOA');
		}else{
			getContainerHtml('repair');
		}
	...
	$("#onlineZx,#onlineZx1").click(function(){
...
function zhixun(){ 
//	$.get("sendChatMsg.do",function(data){
//		pop_up_box.loadWaitClose();
...
$.ajaxSetup({
	contentType : "application/x-www-form-urlencoded;charset=utf-8",
	 ...
	 dataFilter:function(response){
			 if(response.indexOf("<title>登录</title>") > 0||response.indexOf("<title>入口</title>") > 0) {
				alert("登录超时,请重新登录!");
	 ...
	 beforeSend:function(xhr,textStatus){
		 ...
		 if (responseText){
			 if(responseText.indexOf("<title>登录</title>") > 0||responseText.indexOf("<title>入口</title>") > 0) {
