...
function IsPC() {
	...
	$.get("../product/getImgUrl.do",{"item_id":item_id,"com_id":com_id,"customer_id":customer_id},function(data){
		if (data.cps) {
			...
			var name=data.cps[i];
			if (IsPC()) {
				$("#imageGallery").append("<li data-thumb='"+name+"'> <a href='../pc/image-view.html?type=product&url="+item_id+"&cp=cp' target='_blank'> <img src='"+name+"' /></a></li>");
			}else{
				$("#fade").append("<li> <a href='../pc/image-view.html?type=product&url="+item_id+"&cp=cp' target='_blank'> <img src='"+name+"' /></a></li>");
			}
			}
		...
		if (data.xjs) {
			for (var i = 0; i < data.xjs.length; i++) {
				...
				if(name.indexOf("mp4")>=0){
					imgpro.append("<video controls='controls'  preload='none' height='400' width='480' src='"+name+"?ver="+Math.random()+"'></video>");
				}else{
					imgpro.append("<img src='"+name+"?ver="+Math.random()+"'>");
				}
			}
		}
	...
	$("#orderpay").click(function(){
		if(!orderpay){
			setCookieval("orderpay",com_id+"_"+item_id+"_"+$(".num").val());
		...
		$.post("../customer/orderpay.do",{
			...
			if(data.success){
				removeCookie();
				window.location.href="../customer/orderConfirm.do?type=cp";
			}else{
				if(data.msg){
