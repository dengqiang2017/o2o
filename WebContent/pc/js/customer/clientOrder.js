$(function(){
	...
	if (backurl) {
		...
		$(".btn-primary").find("a").click(function(){
			window.location.href=backurl;
			return false;
		});
	}
...
//			$(".order-msg li:eq(3)>span").html(data.ordercount.zje);
		...
		if (data.customerinfo.transport_AgentClerk_Reciever) {
			transport_AgentClerk_Reciever.val(data.customerinfo.HY_style);
		}
		////////////////////////
		...
		if (yck_yue>0) {
			...
			$("tbody:eq(2)").append(ztr);
			///////////2015-10-27 16:45/////////////////
			if (zhye<ddzje) {//总金额小于订单总金额的时候直接使用订单金额,不能在使用预存款
		...
		///////////2015-10-27 16:45/////////////////
		if (zhye>=ddzje) {
			if (zsk_yue<ddzje) {//账上款余额小于订单金额时提示使用预存款
	...
	if(!$.cookie("transport_AgentClerk_Reciever")){
		transport_AgentClerk_Reciever.val("客户自提");
	}
	Kar_Driver.val($.cookie("Kar_Driver"));
	if(!$.cookie("Kar_Driver")){
		Kar_Driver.val("张明");
	}
	Kar_Driver_Msg_Mobile.val($.cookie("Kar_Driver_Msg_Mobile"));
	if(!$.cookie("Kar_Driver_Msg_Mobile")){
		Kar_Driver_Msg_Mobile.val("13112345678");
	}
	Kar_paizhao.val($.cookie("Kar_paizhao"));
	if(!$.cookie("Kar_paizhao")){
		Kar_paizhao.val("川A001002");
	}
