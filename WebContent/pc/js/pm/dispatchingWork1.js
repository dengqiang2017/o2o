	...
	(function(){
		...
		$.ajax({ 
	        ...
	        success: function(data){
	        	...
	        	$("#c_memo").html(data.c_memo);
	        	$("#memo_color").html(data.memo_color);
	        	$("#memo_other").html(data.memo_other);
...
var dispatchingWork = function(){
	//工艺图纸imgUrl
	var imgUrl = "";
	$(".gytz").each(function(n){
		var gytz = $(".gytz")[n];
		imgUrl = imgUrl + $(gytz).find("#imgurl").val()+"_";
	});
	if(imgUrl!=""){
...
var imgUpload = function(data){
	$("#gytz_display").show();
	ajaxUploadFile({
		"uploadUrl":"../upload/uploadImage.do?fileName=imgFile&type=SCPG",
		"msgId":"msg",
		"fileId":"imgFile",
		"msg":"",
		"fid":"",
...
//删除工艺图纸
var delImg = function(data){
	var prop = $(data).parent();
	var imgUrl = prop.find("#imgurl").val();
	pop_up_box.loadWait();
...
var seegytz = function(data){
	...
	var PH = prop.find("#m_PH").html();
	var JHGR = prop.find("#m_clerk_id").html();
	//获取工艺图纸名称
	pop_up_box.loadWait();
	$.get("../pm/seegytz.do", {
		"PH" : PH,
