			...
			function addItem(data) {
				if (data && data.rows.length > 0) {
					...
					$.each(data.rows,function(i, n) {
						...
						if(tabsIndex == 0){
							if(n.pack_unit){
								item.find(".num").val(n.sd_oq/n.pack_unit);
							}else{
							...
							item.find("#ivt_oper_listing").append(""
									+"<span class='p-class' id='ivt_oper_listing_span'>"+n.ivt_oper_listing+"</span>"
									+"<span class='p-class' id='corp_sim_name_span'>"+n.corp_sim_name+"</span>" 
									+"<span class='p-class' id='sd_order_id_span' style='display:none;'>"+n.sd_order_id+"</span>"
									+"<span class='p-class' id='customer_id_span' style='display:none;'>"+n.customer_id+"</span>" 
									+"<span class='p-class' id='seeds_id_span' style='display:none;'>"+n.seeds_id+"</span>");
						}else{
							if(!n.pack_unit){
								n.pack_unit = 1;
							}
							...
							if($(this).find("#item_id").val()!="" && $(this).find("#item_id").val()!=null){
								if($(this).find(".zsum").val()!="" && $(this).find(".zsum").val()!=null){
									dataArr.push(JSON.stringify({
										...
										"customer_id" : $.trim($(this).find("#customer_id_span").html()),
