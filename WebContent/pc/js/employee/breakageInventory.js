$(function(){
			...
			function loadData(page,count){
				...
				if(lia==0){
					...
					url="../employee/getBreInventory.do";
				}
					...
					$.each(data.rows,function(i,n){
						...
						if(lia==0){
							...
							item.find("#pronum").bind("input propertychange blur",function(){
								...
								var sumsi=numformat2(item_cost*num);
								if (sumsi<=0) {
...
//							item.find("#corp_name").html(n.corp_name);  
							...
							item.find("#bs_name").parent().remove();
							item.find("#corpName").parent().remove();
							item.find("#c_memo_b").parent().remove();
						}else{
							...
							item.find("#corpName").html(n.corp_name);  
							item.find("#oper_price").html(n.oper_price);//报损单价
							item.find("#oper_qty").html(n.oper_qty);//报损数量
							item.find("#sum_si").html(n.plan_price);//报损金额
							item.find("#sum_si").html(n.oper_qty*n.oper_price);
							item.find("#ivt_oper_listing").html($.trim(n.ivt_oper_listing));//报损单号
							item.find("#bs_name").html(n.clerk_name);
...
$("#saveOrder").click(function(){
	...
	if (chekcs&&chekcs.length>0) {
		...
		for (var i = 0; i < chekcs.length; i++) {
			...
			var num=item.find("#pronum").val();//报损数量
			var oper_price=$.trim(item.find("#item_cost").val());//报损单价
			var kcNum=$.trim(item.find("#kc").html());//库存数量
			if(num==""||num==0){
				pop_up_box.showMsg("报损数量必须大于0!");
				b=false;
			}
			if(oper_price==""||oper_price=="0"){
				pop_up_box.showMsg("请输入产品报损价格!");
				item.find("#item_cost").focus().select();
				b=false;
			}
			if(num-kcNum>0){
				pop_up_box.showMsg("报损数量大于库存数量!");
				b=false;
			}
		}
		if(b){
			for (var i = 0; i < chekcs.length; i++) {
				...
				if (item_id!="") {
					var oper_price=$.trim(item.find("#item_cost").val());//报损单价
					var oper_qty=item.find("#pronum").val();//报损数量
					var store_struct_id=$.trim(item.find("#store_struct_id").val());//报损仓库
					var corp_id=$.trim(item.find(".corp_id").val());//供应商
					var plan_price=item.find("#sum_si").html();//报损金额
			...
			$("#sure").click(function(){
					...
					$.post("../employee/savebreInventory.do",{
						...
						$("#sure").unbind("click");
						if (data.success) {
							pop_up_box.showMsg("已生成报损单,等待审核!",function(){
								$("#allcheck").html("全选");
...
$("#confirm").click(function(){
	...
	if (chekcs&&chekcs.length>0) {
		for (var i = 0; i < chekcs.length; i++) {
			...
			var oper_qty=$.trim(item.find("#oper_qty").html());//报损数量
			var kcNum=$.trim(item.find("#kc").html());//库存数量
			var itemdata={
					"item_id":item_id, 
					"store_struct_id":store_struct_id,
		...
		if(window.confirm("相同报损单号必须全选,请确认?")){
			pop_up_box.postWait();
			$.post("../employee/confirmBreInv.do",{
				"item_ids":"["+item_ids.join(",")+"]",
			},function(data){
				pop_up_box.loadWaitClose();
				if(data.success){
