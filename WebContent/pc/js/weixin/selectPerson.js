var selectPerson={
				...
				if (n==0) {//选择所有人
					...
					if (checks&&checks.length>0) {
						for (var i = 0; i < checks.length; i++) {
							...
							if ($.trim(tr.find("td:eq(1)").text())!="") {
								name.push(tr.find("td:eq(1)").text());
								weixinID.push(tr.find("td:eq(5)").text());
							}
						}
					}
		...
		},setImg:function(userid){
			$.get("getWeixinImg.do",{
				"userid":userid
			},function(data){
				 $("input[value='"+userid+"']").parents(".chat-mem").find("img").attr("src",data);
			});
		}
}